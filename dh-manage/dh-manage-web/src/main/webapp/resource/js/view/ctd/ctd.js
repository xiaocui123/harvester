$(function () {
    $('#grid').bootstrapTable({
        ...
        uniqueId: "ctdId",             //每一行的唯一标识，一般为主键列
        ...
        columns: [
            ...
            {
                field: "ctdId", title: "序号", width: 40, align: 'center',
                formatter: function (value, row, index) {
                    return index + 1;
                }
            },
            {field: "publishDatasetName", title: "数据集名称", width: 100, valign: 'middle'},
            {field: "ctdName", title: "文件名称", width: 100, valign: 'middle'},
            {field: "ctdGenerator", title: "创建者", width: 80, valign: 'middle'},
            {field: "ctdGenerateTimeStr", title: "创建时间", width: 100, valign: 'middle'},
            {
                field: 'ctdId', title: '操作', width: 120, align: 'center',
                formatter: function (value, row, index) {
                    console.log(row);
                    var gridBtnDownload = '<input type="button" value="下载" onclick="gridBtnDownload(\'' + row.ctdId + '\')"  class="btn btn-primary btn-xs">';
                    if(row.published){
                        ...
                        var gridBtnPublish = '<input type="button" value="发布" onclick="gridBtnPublish(\'' + row.ctdId + '\')"  class="btn btn-danger btn-xs">';
    ...
    $("#file-0a").fileinput({
        language: 'zh', //设置语言
        uploadUrl: path + "/ctd/generate", //上传的地址
    ...
    $('#publishBtn').click(function(){
        ...
        var publish={
            ...
            lstTagName: tags,
            publishResourceType:"ROUTES_CTD"
        };
...
function gridBtnDownload(row) {
    window.open(path + '/ctd/downloadFile?ctdId=' + row);
}
...
function gridBtnDel() {
    ...
    if (selectContent.length > 0) {
        $.each(selectContent,function(index,ctd){
            ids.push(ctd.ctdId);
        });
