$(function () {
    $('#grid').bootstrapTable({
        ...
        uniqueId: "buoyId",             //每一行的唯一标识，一般为主键列
        ...
        columns: [
            ...
            {
                field: "buoyNcId", title: "序号", width: 40, align: 'center',
            ...
            {field: "buoyGenerator", title: "创建者", width: 80, valign: 'middle'},
            {field: "buoyGenerateTimeStr", title: "创建时间", width: 100, valign: 'middle'},
            {
                field: 'buoyNcId', title: '操作', width: 120, align: 'center',
                formatter: function (value, row, index) {
                    console.log(row);
                    var gridBtnDownload = '<input type="button" value="下载" onclick="gridBtnDownload(\'' + row.buoyNcId + '\')"  class="btn btn-primary btn-xs">';
                    if(row.published){
                        ...
                        var gridBtnPublish = '<input type="button" value="发布" onclick="gridBtnPublish(\'' + row.buoyNcId + '\')"  class="btn btn-danger btn-xs">';
    ...
    $("#file-0a").fileinput({
        language: 'zh', //设置语言
        uploadUrl: path + "/buoy/generate", //上传的地址
...
function gridBtnDownload(row) {
    window.open(path + '/buoy/downloadFile?buoyId=' + row);
}
