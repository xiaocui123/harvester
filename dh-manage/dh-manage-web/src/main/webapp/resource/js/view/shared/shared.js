$(function () {
    $('#grid').bootstrapTable({
        url: path + '/shared/query',         //请求后台的URL（*）
        method: 'post',                      //请求方式（*）
        ...
        uniqueId: "shareId",             //每一行的唯一标识，一般为主键列
        ...
        columns: [
            ...
            {
                field: "shareId", title: "序号", width: 40, align: 'center',
                formatter: function (value, row, index) {
                    return index + 1;
                }
            },
            {field: "publishDatasetName", title: "数据集名称", width: 100, valign: 'middle'},
            {field: "sharedName", title: "文件名称", width: 100, valign: 'middle'},
            {field: "sharedGenerator", title: "创建者", width: 80, valign: 'middle'},
            {field: "sharedGenerateTimeStr", title: "创建时间", width: 100, valign: 'middle'},
            {
                field: 'shareId', title: '操作', width: 120, align: 'center',
                formatter: function (value, row, index) {
                    console.log(row);
                    var gridBtnDownload = '<input type="button" value="下载" onclick="gridBtnDownload(\'' + row.shareId + '\')"  class="btn btn-primary btn-xs">';
                    if(row.published){
                        ...
                        var gridBtnPublish = '<input type="button" value="发布" onclick="gridBtnPublish(\'' + row.shareId + '\')"  class="btn btn-danger btn-xs">';
    ...
    $("#file-0a").fileinput({
        language: 'zh', //设置语言
        uploadUrl: path + "/shared/generate", //上传的地址
...
function gridBtnDownload(row) {
    window.open(path + '/shared/downloadFile?sharedId=' + row);
}
...
function gridBtnDel() {
    ...
    if (selectContent.length > 0) {
        $.each(selectContent,function(index,shared){
            ids.push(shared.shareId);
        });
        bootbox.setLocale("zh_CN");
        bootbox.confirm("确定要删除吗？", function (result) {
            if (result) {
                $.ajax({
                    url: path + '/shared/delete',
                    method: 'post',
                    dataType: "json",
                    contentType: 'application/json;charset=UTF-8',
